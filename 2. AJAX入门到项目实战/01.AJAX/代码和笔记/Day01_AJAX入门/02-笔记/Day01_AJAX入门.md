...
1. 什么是 AJAX ? [mdn](https://developer.mozilla.org/zh-CN/docs/Web/Guide/AJAX/Getting_Started)
   ...
   * 使用**浏览器**的 XMLHttpRequest 对象 与**服务器**通信
...
4. 怎么学 AJAX ?
   ...
   * 这里使用一个==第三方库叫 axios==, 后续再学习 XMLHttpRequest 对象了解 AJAX 底层原理
...
4. 什么是 http 协议 ?
   ...
   * 叫**超文本传输协议**，规定了浏览器和服务器传递数据的格式（而格式具体有哪些稍后我们就会学到）
...
1. 想要提交数据，先来了解什么是请求方法
   ...
   * 请求方法是一些固定单词的英文，例如：GET（默认），POST，PUT，DELETE，PATCH（这些都是http协议规定的），每个单词对应一种对服务器资源要执行的操作

     ![image-20230220130833363](images/image-20230220130833363.png)

     

   * 前面我们获取数据其实用的就是GET请求方法，但是==axios内部设置了默认请求方法就是GET（不区分大小写）==，我们就没有写

   * 但是提交数据需要使用POST请求方法, 提交数据写==在data{}== 里，若数据格式为js对象，axios会自动转换成json格式
...
3. 这里的格式包含：
   ...
   * 请求行（Line1）：请求方法，URL，协议
   * 请求头（Line2-11）：以键值对的格式携带的附加信息，比如：Content-Type（指定了本次传递的内容类型json）
   * 空行（Line12）：分割请求头，空行之后的是发送给服务器的资源
   * 请求体（LIne13）：发送的资源
...
querySelector不用一个个去选form内input的值，而是传一个form给serialize函数快速收集
